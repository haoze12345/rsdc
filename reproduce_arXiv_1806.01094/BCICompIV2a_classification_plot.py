...
def plot(parameters={}):
    ...
    basepath = './BCICompIV2a/' + parameters['expname']
    ...
    ax.axhline(1 / 4, color='gray', linewidth=1, zorder=1)

    for posA, posB, exp in zip(positions[0], positions[-1], exps):
        no_trials_heldout = (9 - exp) * 576
        CIinterval = np.asarray(
            binom.interval(.95, no_trials_heldout, 1 / 4)) / \
            no_trials_heldout
        print(CIinterval)
