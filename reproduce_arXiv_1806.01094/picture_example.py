...
from rnojd_corica import CoroICA,UwedgeICARNOJD
...
def rffdiag_wrapper(X, pd=True, max_iter = 10):
...
images = [ image / np.linalg.norm(image) for image in images]
#noise_1 = np.random.normal(0,1,size = images[1].shape)
#images.append( noise_1 / np.linalg.norm(noise_1))
#noise_2 = np.random.chisquare(3,size = images[1].shape)
#images.append(1e-6 * noise_2 / np.linalg.norm(noise_2))
...
print("Hfull Shape", Hfull.shape)
Hfull = 0
noise_mixxing_shape = (len(images), 8)
Hmat = np.random.normal(0, 1, noise_mixxing_shape).dot(Hfull)

Xmat = A.dot(Smat)


images_g0 = [scale_image((Smat)[imgind, :][grid_re])
...
timelags = [0]
partitionsize = 300
rnojd_ica =  UwedgeICARNOJD(partitionsize=partitionsize,
                      timelags=timelags,
                      instantcov=False)
rffiag_ica = UwedgeICARNOJD(partitionsize=partitionsize,
                      timelags=timelags,
                      instantcov=False)
uwedgeica = UwedgeICARNOJD(partitionsize=partitionsize,
                      timelags=timelags,
                      instantcov=False)
rnojd_ica.fit(Xmat.T, algorithm= rnojd_wrapper)
rffiag_ica.fit(Xmat.T, algorithm=rffdiag_wrapper)

uwedgeica.fit(Xmat.T, use_uwedge=True, random_uwedge=False)
unmixings = [
