...
"""
from coroica.utils import autocov,rigidgroup
from coroica.utils import rigidgroup as rigidpartition_uwedge
...
sys.path.append("..")
from rnojd import rnojd, manopt_rnojd, ffdiag_rnojd, offdiag_frobenius_square
...
class CoroICA(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, group_index=None, partition_index=None):
        ...
        Rx0 = np.cov(X)
        #covmats = np.append(covmats,[Rx0],axis = 0)
        #print("Shape of the matrice to be JD: ", covmats.shape)

        # joint diagonalisation

        self.V_ = ffdiag_rnojd(covmats, trials=5,max_iter=10, trivial_init=False)
        #self.V_ ,_ = rjd(covmats).T
        # normalise V
...
class UwedgeICARNOJD(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None, partition_index=None, algorithm = rnojd, use_uwedge = False, random_uwedge = False):
        """Fit the model
        ...
        # generate partition index as needed
        if partition_index is None and self.partitionsize is None:
            partition_indices = [rigidpartition_uwedge(
                n,
                np.max([dim, n // 2]))]
        elif partition_index is None and type(self.partitionsize) == list:
            partition_indices = [rigidpartition_uwedge(n, partsize)
                                 for partsize in self.partitionsize]
        elif partition_index is None:
            partition_indices = [rigidpartition_uwedge(n, self.partitionsize)]
        else:
            partition_indices = [partition_index]
        ...
        '''for i in range(covmats.shape[0]):
            print(np.linalg.norm(covmats[i] -covmats[i].T))
            covmats[i] = (covmats[i] + covmats[i].T)/2'''
        if not use_uwedge:
            self.V_  = algorithm(covmats)
        else:
            init = None
            if random_uwedge:
                init = rnojd(covmats).T
        ...
        self.V_ =   (self.V_.T  / np.linalg.norm(self.V_.T ,axis=0)).T
        #normalise V
        ...
        if self.n_components is not None:
            self.V_ = self.V_[:self.n_components, :]'''
        #print("Final error:", offdiag_frobenius_square(self.V_ @ covmats @ self.V_.T ))
...
class CoroICARNOJD(BaseEstimator, TransformerMixin):
    def __init__(self,
                 n_components=None,
                 n_components_uwedge=None,
                 rank_components=False,
    ...
    def fit(self, X, y=None, partition_index=None, algorithm = rnojd, use_uwedge = False, random_uwedge = False, group_index=None):
