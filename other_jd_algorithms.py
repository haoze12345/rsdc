...
from numpy import diag, dot, eye, zeros, diagonal
...
def ffdiag_update(R_tau,ortho):
    ...
    Dk = diagonal(R_tau,axis1=1,axis2=2)
    
    Ek =  R_tau- np.array([diag(d) for d in Dk])

    W = zeros((dim,dim))
    if ortho is False:
        z = Dk.T @ Dk
        y = zeros(W.shape)        
        y = np.einsum('ijk,ik->jk',Ek,Dk)

        z_y = np.multiply(z, y)
        z_y_t = np.multiply(z,y.T)
        diag_z_times_y_upper = diag(diag(z))@y
        diag_z_times_y_lower = diag_z_times_y_upper.T
        nominator_upper = z_y_t - diag_z_times_y_upper
        nominator_lower = z_y - diag_z_times_y_lower
        z_square = np.square(z)
        z_diag_outer = np.outer(diag(z),diag(z))
        denominator =  z_diag_outer - z_square
        #nominator_upper = np.triu(nominator_upper)
        #denominator = np.triu(denominator)
        #nominator_lower = np.triu(nominator_lower)
        np.seterr(divide='ignore', invalid='ignore')
        upper_triangular = np.triu(np.divide(nominator_upper,denominator),k=1)
        lower_triangular = np.triu(np.divide(nominator_lower,denominator),k=1)
        W = upper_triangular + lower_triangular.T
        # compute W
...
def ffdiag(X, eps = 1.0e-08, max_iter = 100, V0 = None):
    '''
    Adapted from https://github.com/archimonde1308/pybss/blob/master/bss.py
